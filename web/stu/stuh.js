...
function refreshInfoMap(){
    $.ajax({
        ...
        success:function (data) {
            if(data.infoMap!="null"){
                var mypoints=data.infoMap.myScore;
                $("#mypoints").html(mypoints+"分")
                var onlineDisNum=data.infoMap.onlineDiscuss
                $("#onliNum").html(onlineDisNum+"人")
                var warnNums=data.infoMap.warnNums
                $("#warnNum").html(warnNums+"个")
                var warnList=data.infoMap.warnList
                var testing="无"
                if (data.infoMap.lastExam==null){
                    testing="无"
                    $("#lexamtime").html("没有正在进行的测验")
                }else {
                    testing="有"
                    $("#lexamtime").html("开始时间："+data.infoMap.lastExam.startTimes.substr(0,19))
                    $("#enterexam").attr("href","/stu/takeExam?paperid="+data.infoMap.lastExam.paper.pid+"&examTime="+data.infoMap.lastExam.examTime)
                }
                ...
                var scoreList=data.infoMap.scoreReportList
                ss2=""
